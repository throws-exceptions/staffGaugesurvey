...
var Methods = {
  ...
  checkSelectionStatus: function checkSelectionStatus() {
    ...
    if (!checkStrictly) {
      if (property) {
        this.isAllSelected = afterFullData.length && afterFullData.every(checkMethod ? function (row) {
          return !checkMethod({
            row: row
  ...
  if (opts.isHeader) {
    html += "<thead><tr>".concat(columns.map(function (column) {
      ...
      if (column.type === 'checkbox' || column.type === 'selection') {
        return "<td class=\"".concat(classNames.join(' '), "\"><div ").concat(isPrint ? '' : "style=\"width: ".concat(column.renderWidth, "px\""), "><input type=\"checkbox\" ").concat(isAllSelected ? 'checked' : '', "><span>").concat(cellTitle, "</span></div></td>");
      }
...
function getCheckboxRangeResult($table, params, targetTrElem, moveRange) {
  ...
  if ($table.scrollYLoad) {
    var afterFullData = $table.afterFullData,
        scrollYStore = $table.scrollYStore;

    var _rowIndex = $table._getRowIndex(params.row);
  ...
  methods: {
    ...
    handleCheckboxRangeEvent: function handleCheckboxRangeEvent(evnt, params) {
      ...
      if (['checkbox', 'selection'].indexOf(column.type) > -1) {
        ...
        var disX = evnt.clientX;
        var disY = evnt.clientY;
        var bodyWrapperElem = elemStore["".concat(column.fixed || 'main', "-body-wrapper")];
        var checkboxRangeElem = elemStore["".concat(column.fixed || 'main', "-body-checkRange")];
        var domMousemove = document.onmousemove;
        var domMouseup = document.onmouseup;
        var trElem = cell.parentNode;
        var selectRecords = this.getCheckboxRecords();
